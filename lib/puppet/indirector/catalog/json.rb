...
class Puppet::Resource::Catalog::Json < Puppet::Indirector::JSON
  ...
  def save(request)
    ...
    Puppet::Util.replace_file(filename, 0660) {|f| f.print Puppet::Util::Encrypt.encrypt(to_json(request.instance).force_encoding(Encoding::BINARY), Puppet::Util::Artifacts::CATALOG)}
  ...
  def load_json_from_file(file, key)
    ...
    begin
      json = Puppet::Util::Encrypt.decrypt(Puppet::FileSystem.read(file, :encoding => Encoding::BINARY), Puppet::Util::Artifacts::CATALOG)
    rescue Errno::ENOENT
      return nil
    rescue => detail
