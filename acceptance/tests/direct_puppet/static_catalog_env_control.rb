...
agents.each do |agent|
  ...
  @secure_artifacts[hn] = "#{on(agent, puppet('config', 'print', 'secure_artifacts')).stdout}"
...
teardown do
  ...
  # Restore the original secure_artifacts setting back
  agents.each do |agent|
    hn = agent.node_name
    on(agent, "puppet config set secure_artifacts #{@secure_artifacts[hn]}")
  end
end
...
with_puppet_running_on master, @master_opts, @coderoot do
  agents.each do |agent|
    on(agent, puppet('config set secure_artifacts false'))
    hn = agent.node_name
